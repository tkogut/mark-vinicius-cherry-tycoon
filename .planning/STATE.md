...
**Current focus:** Phase 3 — Core Economic Lever Fixes (Phase 2 closed 2026-08-07 as an agreed vertical slice; the Sports Patron's orchard impact and league simulation are tracked as SPORTS-05/06, not silently dropped)
...
**Velocity:**
- Total plans completed: 9 (Phase 1: 1, Phase 1.1: 1, Phase 2: 1, Phase 9: 1 unplanned, Phase 10: 5)
...
- **Sports Patron scope (2026-08-07):** the GDD addendum opens with "should remain paused until Manager decides to implement it" and specifies a module several times larger than Phase 2's success criteria. User chose a **vertical slice** — clubs, purchases, UI — over the full spec or a further deferral. Ownership modelled as **single-patron** because `Types.FootballClub` carries one `ownerId`/`ownershipPercent` and the GDD frames the player as *the* village benefactor; a multi-shareholder market would need a type change. Club catalogue kept as **code, not stable state**, so the endpoint stays a `query` and upgrades need no migration.

- **Agent instruction layer (2026-08-07):** adopted four patterns from open-mercato's `AGENTS.md` (github.com/open-mercato/open-mercato), narrowed to a solo project: a **Task Router** in `CLAUDE.md` mapping task → files to read; **Always / Ask First / Never** boundary labels replacing prose rules; `.planning/LESSONS.md` as a self-improvement loop (mistakes as rules, seeded from real corrections in this repo's history); and one ordered validation gate, `./validate.sh {fast|full|backend}`, replacing a sequence I reconstructed from memory each time. `agentDocs.test.ts` keeps the layer honest (every route resolves, byte budget, tiers match `package.json`) — verified by injecting a dead route. Explicitly **not** adopted: their PR label taxonomy and QA gates (need a team and a QA reviewer), multi-tenant scoping, monorepo package layout, DI/zod/CrudForm conventions, and the full deprecation protocol (no third-party consumers).
...
- ~~`MAINT-02` repair pricing made servicing a losing move~~ → **fixed 2026-08-07**. The flat 500/level fee could never pay back for 6 of the 8 priced asset types (the break-even is 0.8x spec upkeep per year, i.e. spec > 625 — so Tractor was negative too, correcting the figure first recorded). Repair now costs half the accumulated wear, which makes the trade identical for every asset type and independent of level; assets at spec cost nothing instead of a 500 fee for a no-op. Verified on a live replica (Sprayer 312 -> 240 for 36 PLN, second call free).
...
- Clear the remaining flagged defects above (`GEO-07` folds naturally into Phase 5; `ECON-PARITY-01` needs an economy-balance decision, not code fixes)
