        ...
        <div className="mechanical-hull w-full h-full flex flex-col justify-center relative isolate overflow-hidden">
            ...
            <div className="mahogany-plate brass-rim w-full h-full z-10 shadow-2xl flex items-center justify-between px-4 md:px-8">
                ...
                <div className="flex-1 max-w-sm">
                    <div className="bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl p-4 brass-rim shadow-[inset_0_0_20px_rgba(0,0,0,0.8)]">
                        <h3 className="hull-header text-sm lg:text-base mb-3 text-center tracking-[0.2em]">Diagnostics</h3>
                        <div className="grid grid-cols-2 gap-2 text-center">
                            <div className="bg-white/5 rounded-lg p-2 border border-white/5">
                                <p className="text-slate-400 text-[10px] uppercase font-mono tracking-widest leading-none mb-1">Phase</p>
                                <p className="text-amber-400 font-bold text-sm leading-none drop-shadow-md">
                                    {farm?.currentPhase ? Object.keys(farm.currentPhase)[0] : 'Idle'}
                                </p>
                            </div>
                            <div className="bg-white/5 rounded-lg p-2 border border-white/5">
                                <p className="text-slate-400 text-[10px] uppercase font-mono tracking-widest leading-none mb-1">Treasury</p>
                                <p className="text-amber-400 font-bold text-sm leading-none drop-shadow-md">
                                    {farm ? Number(farm.cash).toLocaleString() : 0} <span className="text-[10px]">PLN</span>
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex flex-col items-center justify-center gap-2 z-10 px-4 -mt-2">
                    <div className="scale-75 md:scale-90 lg:scale-100 origin-center transition-transform">
                        <ImperialDial
                            value={enginePower}
                            min={0}
                            max={100}
                ...
                <div className="flex-1 max-w-sm flex justify-end gap-2 md:gap-6">
                    <div className="flex flex-col items-center bg-black/40 p-3 rounded-2xl brass-rim backdrop-blur-sm scale-75 md:scale-90 lg:scale-100 origin-bottom-right">
                        <EmeraldGauge value={storagePercentage > 100 ? 100 : storagePercentage} label="Reserves" />
                        <div className="text-emerald-400 font-mono font-bold text-center text-sm md:text-base drop-shadow-[0_0_8px_rgba(80,200,120,0.6)] mt-[-10px]">
